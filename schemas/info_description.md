...
- **Tag Protection** — Lock individual tags with a password so they are invisible to unauthorized readers. *(tag-scoped — applies to a specific tag by EPC)*
...
> **Reader-scoped** features (FastID, TagFocus) change the reader's general inventory behavior and affect all tags. **Tag-scoped** features (Tag Protection, Tag Quieting) target individual tags identified by their EPC.
...
3. **Configure features** — Use the `set_impinjGen2X` command to stage the Gen2X features you want to enable (e.g., Tag Protection, FastID, TagFocus).
...
> **Feature persistence:** Reader-scoped settings (FastID, TagFocus) and tag-scoped settings (Tag Protection, Tag Quieting) are staged via `set_impinjGen2X` and applied when the radio starts with `applyImpinjGen2X: true`. Staged configurations persist across stop/start cycles as long as the MQTT session remains connected. A reader reboot or MQTT disconnect clears all staged configurations, and you must re-send `set_impinjGen2X` to restore them.